...
- `run_backend_input.py`
  - 直接从负载输入运行仿真，并在结束后自动写出快照 JSON 与 SQLite 数据库
- `create_frontend_database.py`
  - 将已有输入 JSON 和快照 JSON 重新构建为 SQLite 数据库；通常仅在需要从现有文件重新生成 db 时使用
- `webapp.py`
  - 暴露 FastAPI 服务和仿真端点
...
当默认示例输入缺失时，`run_backend_input.py` 会自动创建 `examples/backend_sample_input.json`，并且即使从未使用内置调试 viewer 页面，也会将最新的默认快照写入 `outputs/backend_sample_output.json`，同时将输入与最终快照写入 `psychology_backend_frontend.db`。如需指定数据库路径，可使用 `--db` 参数，例如 `python run_backend_input.py --db outputs/custom_run.db`。
