...
async def run_from_payload_async(
    ...
    storage = SimulationStorage(output_dir=os.path.join(str(PARENT_ROOT), BACKEND_IO.outputs_dir_name))
    env = make(
        agents=agents,
    ...
    # 6) Attach extra debug metadata, then export the same snapshot that will
    # be returned and persisted to SQLite.
    snapshot = env.snapshot()
    snapshot["_debug"] = snapshot_debug_meta(snapshot, payload)
    env.export(filename=BACKEND_IO.exported_snapshot_name, snapshot=snapshot)
